...
public class CameraFollow : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		...
		Vector3 standardPos = player.position + relativeCameraPosition;
		
		// The abovePos is directly above the player at the same distance as the standard position.
		Vector3 abovePos = player.position + Vector3.up * relativeCameraPositionMag;
	...
	bool ViewingPosCheck (Vector3 checkPos)
	{
		...
		if(Physics.Raycast(checkPos, player.position - checkPos, out hit, relativeCameraPositionMag))
